...
export class ProseManager {
     ...
     */
    async runProseGeneration(novelId, chapterId, scenePlanVersionId, previousSnapshotId, jobId) {
        ...
        if (scenePlanVersion.chapterId !== chapterId) {
            throw new Error(`Cross-version contamination rejected: ScenePlanVersion ${scenePlanVersionId} belongs to chapter ${scenePlanVersion.chapterId}, not ${chapterId}`);
        }
        // Wrap provider with usage proxy if we have a jobId
        const originalProvider = this.handler.provider;
        if (jobId) {
            this.handler.provider = new LLMUsageProxy(originalProvider, originalProvider.getProviderName(), novelId, 'PROSE_GENERATION', chapterId, jobId);
        }
        try {
            ...
            for (const scene of scenePlanVersion.scenes) {
                ...
                if (scene.scenePlanVersionId !== scenePlanVersionId) {
                    throw new Error(`Cross-version contamination rejected: Scene ${scene.id} belongs to ScenePlanVersion ${scene.scenePlanVersionId}, not ${scenePlanVersionId}`);
                }
            ...
            // ----------------------------------------------------------------
            if (newVersionId) {
                const gateReport = await GenerationQualityGate.runGate(novelId, chapterId, newVersionId, {
                ...
                if (gateReport.recommendation === 'BLOCK') {
                    ...
                    throw new Error(`Quality gate BLOCKED promotion for chapter ${chapter.number}: ${gateReport.conflicts
            ...
            if (stateDeltas.length > 0) {
                await ChapterMemoryManager.promoteStoryState(novelId, chapter.number, stateDeltas).catch((err) => {
            ...
            // ----------------------------------------------------------------
            await ChapterMemoryManager.createMemory(novelId, chapterId, chapter.number, stateDeltas, `Chapter ${chapter.number}${chapter.title ? `: ${chapter.title}` : ''} — ${generatedScenes.length} scenes generated`, {
                locations: scenePlanVersion.scenes.map((s) => s.location).filter(Boolean),
            ...
            // ----------------------------------------------------------------
            const totalWordCount = generatedScenes.reduce((acc, s) => acc + (s.wordCount ?? 0), 0);
