...
describe('Phase 1 RLS Ownership (Real)', () => {
  ...
  beforeAll(async () => {
    ...
    if (!connectionString) {
      console.warn('Skipping Phase 1 RLS tests: DATABASE_URL is not set.')
      return
    }
    ...
    try {
      ...
      throw new Error(`Could not connect to Postgres database for Phase 1 RLS tests: ${describeDbError(error)}`)
    }
  })
