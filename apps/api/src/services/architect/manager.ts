...
import { ArchitectStage, ArchitectStatus, STAGE_REGISTRY, StageDefinition } from '@ane/core';
import { LLMUsageProxy } from '../generation/LLMUsageProxy.js';

export class ArchitectManager {
  private provider: LLMProvider;
  private handlers: Map<ArchitectStage, Handlers.StageHandler<any>>;
  ...
  getDownstreamStages(targetStage: ArchitectStage): ArchitectStage[] {
    const queue = [targetStage];
    const visited = new Set<ArchitectStage>();
    const downstream: ArchitectStage[] = [];
    ...
    while (queue.length > 0) {
      ...
      for (const [stage, def] of Object.entries(STAGE_REGISTRY) as [ArchitectStage, StageDefinition][]) {
